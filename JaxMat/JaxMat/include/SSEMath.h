...
namespace JaxMat
{
	...
	*/
	__forceinline void JAXMAT_API MultVectorMatrix(const float* Matrix4, const float* Vector4, float* ResultOut)
	{
		JAX_ASSERT(JaxMem::FMemory::IsAligned(Matrix4, 16));
		JAX_ASSERT(JaxMem::FMemory::IsAligned(Vector4, 16));
		JAX_ASSERT(JaxMem::FMemory::IsAligned(ResultOut, 16));
	...
	*/
	__forceinline void JAXMAT_API Vector4FloatToInt(const float* Floats, int32_t* Results)
	{
		JAX_ASSERT(JaxMem::FMemory::IsAligned(Floats, 16));
	...
	* @param Results - Memory location to place the converted values.
	*/
	__forceinline void JAXMAT_API Vector4IntToFloat(const int32_t* Ints, float* Results)
	{
	...
	*/
	__forceinline void JAXMAT_API MultMatrixMatrix(const float* Mat1, const float* Mat2, float* ResultOut)
	{
		JAX_ASSERT(JaxMem::FMemory::IsAligned(Mat1, 16));
		JAX_ASSERT(JaxMem::FMemory::IsAligned(Mat2, 16));
		JAX_ASSERT(JaxMem::FMemory::IsAligned(ResultOut, 16));
	...
	* @param ResultOut - Location of the result.
	*/
	__forceinline void JAXMAT_API AddMatrixMatrix(const float* Mat1, const float* Mat2, float* ResultOut)
	{
		JAX_ASSERT(JaxMem::FMemory::IsAligned(Mat1, 16));
		JAX_ASSERT(JaxMem::FMemory::IsAligned(Mat2, 16));
		JAX_ASSERT(JaxMem::FMemory::IsAligned(ResultOut, 16));
	...
	* @param ResultOut - Location of the result.
	*/
	__forceinline void JAXMAT_API MultComponentMatrixMatrix(const float* Mat1, const float* Mat2, float* ResultOut)
	{
		JAX_ASSERT(JaxMem::FMemory::IsAligned(Mat1, 16));
		JAX_ASSERT(JaxMem::FMemory::IsAligned(Mat2, 16));
		JAX_ASSERT(JaxMem::FMemory::IsAligned(ResultOut, 16));
	...
	*/
	template <typename T>
	__forceinline void JAXMAT_API Dot4Product(const T* Vec1, const T* Vec2, T* ResultOut)
	{
	...
	template <>
	__forceinline void JAXMAT_API Dot4Product(const float* Vec1, const float* Vec2, float* ResultOut)
	{
		JAX_ASSERT(JaxMem::FMemory::IsAligned(Vec1, 16));
		JAX_ASSERT(JaxMem::FMemory::IsAligned(Vec2, 16));
		JAX_ASSERT(JaxMem::FMemory::IsAligned(ResultOut, 16));
	...
	template <>
	__forceinline void JAXMAT_API Dot4Product(const int32_t* Vec1, const int32_t* Vec2, int32_t* ResultOut)
	{
		JAX_ASSERT(JaxMem::FMemory::IsAligned(Vec1, 16));
		JAX_ASSERT(JaxMem::FMemory::IsAligned(Vec2, 16));
		JAX_ASSERT(JaxMem::FMemory::IsAligned(ResultOut, 16));
	...
	*/
	...
	__forceinline void JAXMAT_API Dot4ProductBatchAdd(const float* PrimaryVec, const float SecondaryVecs[Count][4], float* ResultOut)
	{
		JAX_ASSERT(JaxMem::FMemory::IsAligned(PrimaryVec, 16));
		JAX_ASSERT(JaxMem::FMemory::IsAligned(SecondaryVecs, 16));
		JAX_ASSERT(JaxMem::FMemory::IsAligned(ResultOut, 16));
	...
	*/
	template <uint32_t Count>
	__forceinline void JAXMAT_API Dot4ProductBatch(const float* PrimaryVec, const float SecondaryVecs[Count][4], float ResultsOut[Count])
	{
		JAX_ASSERT(JaxMem::FMemory::IsAligned(PrimaryVec, 16));
		JAX_ASSERT(JaxMem::FMemory::IsAligned(SecondaryVecs, 16));
		JAX_ASSERT(JaxMem::FMemory::IsAligned(ResultOut, 16));
	...
	*/
	template <typename T>
	__forceinline void JAXMAT_API AddVectorVector(const T* Vec1, const T* Vec2, T* ResultOut)
	{
	...
	template <>
	__forceinline void JAXMAT_API AddVectorVector(const float* Vec1, const float* Vec2, float* ResultOut)
	{
		JAX_ASSERT(JaxMem::FMemory::IsAligned(Vec1, 16));
		JAX_ASSERT(JaxMem::FMemory::IsAligned(Vec2, 16));
		JAX_ASSERT(JaxMem::FMemory::IsAligned(ResultOut, 16));
	...
	template <>
	__forceinline void JAXMAT_API AddVectorVector(const int32_t* Vec1, const int32_t* Vec2, int32_t* ResultOut)
	{
		JAX_ASSERT(JaxMem::FMemory::IsAligned(Vec1, 16));
		JAX_ASSERT(JaxMem::FMemory::IsAligned(Vec2, 16));
		JAX_ASSERT(JaxMem::FMemory::IsAligned(ResultOut, 16));
	...
	*/
	template <typename T>
	__forceinline void JAXMAT_API SubtractVectorVector(const T* Vec1, const T* Vec2, T* ResultOut)
	{
		// default to FPU for integer types for now
		ResultOut.x = Vec1.x - Vec2.x;
		ResultOut.y = Vec1.y - Vec2.y;
	...
	template <>
	__forceinline void JAXMAT_API SubtractVectorVector(const float* Vec1, const float* Vec2, float* ResultOut)
	{
		JAX_ASSERT(JaxMem::FMemory::IsAligned(Vec1, 16));
		JAX_ASSERT(JaxMem::FMemory::IsAligned(Vec2, 16));
		JAX_ASSERT(JaxMem::FMemory::IsAligned(ResultOut, 16));
	...
	template <>
	__forceinline void JAXMAT_API SubtractVectorVector(const int32_t* Vec1, const int32_t* Vec2, int32_t* ResultOut)
	{
		JAX_ASSERT(JaxMem::FMemory::IsAligned(Vec1, 16));
		JAX_ASSERT(JaxMem::FMemory::IsAligned(Vec2, 16));
		JAX_ASSERT(JaxMem::FMemory::IsAligned(ResultOut, 16));
	...
	*/
	template <typename T>
	__forceinline void JAXMAT_API MultVectorVector(const T* Vec1, const T* Vec2, T* ResultOutt)
	{
		// default to FPU for integer types for now
		ResultOut.x = Vec1.x * Vec2.x;
		ResultOut.y = Vec1.y * Vec2.y;
	...
	template <>
	__forceinline void JAXMAT_API MultVectorVector(const float* Vec1, const float* Vec2, float* ResultOut)
	{
		JAX_ASSERT(JaxMem::FMemory::IsAligned(Vec1, 16));
		JAX_ASSERT(JaxMem::FMemory::IsAligned(Vec2, 16));
		JAX_ASSERT(JaxMem::FMemory::IsAligned(ResultOut, 16));
	...
	template <>
	__forceinline void JAXMAT_API MultVectorVector(const int32_t* Vec1, const int32_t* Vec2, int32_t* ResultOut)
	{
		JAX_ASSERT(JaxMem::FMemory::IsAligned(Vec1, 16));
		JAX_ASSERT(JaxMem::FMemory::IsAligned(Vec2, 16));
		JAX_ASSERT(JaxMem::FMemory::IsAligned(ResultOut, 16));
	...
	*/
	template <typename T>
	__forceinline void JAXMAT_API DivideVectorVector(const T* Vec1, const T* Vec2, T* ResultOut)
	{
		// default to FPU for integer types for now
		ResultOut.x = Vec1.x / Vec2.x;
		ResultOut.y = Vec1.y / Vec2.y;
	...
	template <>
	__forceinline void JAXMAT_API DivideVectorVector(const float* Vec1, const float* Vec2, float* ResultOut)
	{
		JAX_ASSERT(JaxMem::FMemory::IsAligned(Vec1, 16));
		JAX_ASSERT(JaxMem::FMemory::IsAligned(Vec2, 16));
		JAX_ASSERT(JaxMem::FMemory::IsAligned(ResultOut, 16));
	...
	*/
	__forceinline void JAXMAT_API MultQuatQuat(const FQuaternion& Lhs, const FQuaternion& Rhs, FQuaternion& ResultOut)
	{
