...
namespace JaxMem
{
	...
	*/
	class JAXMEM_API FPoolAllocator
	{
	public:
		/**
		* Ctor
		...
		*/
		...
		{
			JAX_ASSERT(0 < BlockSize && "BlockSize must be larger that 0");
			JAX_ASSERT(ElementSize >= sizeof(PoolElement) && "ElementSize must at least the size of a standard pointer type.");
...
#undef min
		...
		~FPoolAllocator()
		{
			JAX_ASSERT(mObjectsConstructed == 0 && "All objects should be back in the pool on destruction.");
		...
		*/
		void Free(void* Data)
		{
			JAX_ASSERT(mObjectsConstructed > 0);
			mObjectsConstructed--;
