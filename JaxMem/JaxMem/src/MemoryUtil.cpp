...
namespace JaxMem
{
	namespace FMemory
	{
		...
		{
			JAX_ASSERT(Alignment >= 1 && "Aligned allocation shouldn't be less than 1 byte.");
			JAX_ASSERT(Alignment <= 128 && "Aligned allocations over 128 bytes not supported.");
			JAX_ASSERT((Alignment & (Alignment - 1)) == 0x0 && "Alignment must be a power of 2.");
			...
			JAX_ASSERT(MemoryAdjustment < 256);
